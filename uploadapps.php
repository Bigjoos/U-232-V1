...
$lang = array_merge( load_language('global'), load_language('uploadapps') );
...
if (!$action || $action == "show") {
    ...
    if ($count == 0) {
        ...
        while ($arr = mysql_fetch_assoc($res)) {
            ...
            $HTMLOUT .="<tr>
            <td>{$elapsed}</td>
            <td><a href='?action=viewapp&amp;id=$arr[id]'>{$lang['uploadapps_viewapp']}</a></td>
            <td><a href='{$TBDEV['baseurl']}/userdetails.php?id=$arr[uid]'>$arr[username]</a></td>
            <td>{$membertime}</td>
...
if ($action == "acceptapp") {
    ...
    sql_query("UPDATE users SET class = 3, modcomment = " . sqlesc($modcomment) . " WHERE id=$arr[uid] AND class < ".UC_MODERATOR."") or sqlerr(__FILE__, __LINE__);
    ...
    mysql_query("INSERT INTO messages(sender, receiver, added, msg, subject, poster) VALUES(0, $subarr[id], $dt, $msg1, $subject, 0)") or sqlerr(__FILE__, __LINE__);
...
if ($action == "rejectapp") {
    ...
    mysql_query("INSERT INTO messages(sender, receiver, added, msg, subject, poster) VALUES(0, $arr[uid], $dt, $msg, $subject, 0)") or sqlerr(__FILE__, __LINE__);
    $mc1->delete_value('new_uploadapp_');
...
//== Delete applications
    ...
    mysql_query("DELETE FROM uploadapp WHERE id IN (".join(",",$_POST['deleteapp']).") ") or sqlerr(__FILE__, __LINE__);
    $mc1->delete_value('new_uploadapp_');
