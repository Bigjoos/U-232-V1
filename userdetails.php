...
function maketable($res)
    ...
    $mc1->cache_value('user'.$id, $user, $TBDEV['expires']['user_cache']);
    }
  ...
  //=== delete H&R
    ...
    else
    {
      ...
      $mc1->cache_value('torrent_comments_'.$id, $torrentcomments, $TBDEV['expires']['torrent_comments']);
      }
      //==posts
      ...
      $mc1->cache_value('forum_posts_'.$id, $forumposts, $TBDEV['expires']['forum_posts']);
      }
    ...
    if ($ret === false) {
        ...
        while ($row = mysql_fetch_assoc($res))
            $ret[] = $row;
        $mc1->cache_value('countries::arr', $ret, $TBDEV['expires']['user_flag']);
    }
    return $ret;
    }
    ...
    $country = "<td class='embedded'><img src=\"{$TBDEV['pic_base_url']}flag/{$cntry['flagpic']}\" alt=\"". htmlspecialchars($cntry['name']) ."\" style='margin-left: 8pt' /></td>";
    break;
    }
    ...
    if ($user["parked"] == 'yes')
      ...
      $mc1->cache_value('Friends_'.$id, $friends, $TBDEV['expires']['user_friends']);
      }
      ...
      $mc1->cache_value('Blocks_'.$id, $blocks, $TBDEV['expires']['user_blocks']);
      }
    ...
    $mc1->cache_value('shit_list_'.$id, $shit_list, $TBDEV['expires']['shit_list']);
    }
   ...
   $HTMLOUT.=" [ " . mkprettytime($donoruntil - TIME_NOW) . " ] To go...</b><font size=\"-2\"> To re-new your donation click <a class='altlink' href='{$TBDEV['baseurl']}/donate.php'>Here</a>.</font><br /><br />\n";
    ...
    $mc1->cache_value('hash1_'.$id, $hash1, $TBDEV['expires']['user_hash']);
    }
    $arr = $hash1;
    if ($arr['hash1'] != '') { 
    $HTMLOUT.="<tr><td class='rowhead'>Login Link<br /><a href='createlink.php?action=reset&amp;id=".$CURUSER['id']."' target='_blank'>Reset Link</a></td><td align='left'>{$TBDEV['baseurl']}/pagelogin.php?qlogin=".$arr['hash1']."</td></tr>";
    } else { 
    ...
    if ($CURUSER['class'] >= UC_STAFF){
    $un = $user["username"];
    $HTMLOUT .= "<tr><td class='rowhead' width='1%'>{$lang['userdetails_flush']}</td><td align='left' width='99%'>".("{$lang['userdetails_flush1']}<a href='{$TBDEV['baseurl']}/staffpanel.php?tool=flush&amp;action=flush&amp;id=$id'><b>".htmlspecialchars($un)."</b></a>\n")."</td></tr>";
    }
    ...
    $iphistory = $mc1->get_value('ip_history_'.$id);
        if ($iphistory === false) {
            ...
            $mc1->cache_value('ip_history_'.$id, $iphistory, $TBDEV['expires']['iphistory']);
        }
        if (isset($addr))
        if ($CURUSER['id'] == $id || $CURUSER['class'] >= UC_STAFF)
        $HTMLOUT .= "<tr><td class='rowhead'>{$lang['userdetails_address']}</td><td align='left'>{$addr}{$iphistory['use']}</td></tr>\n";

        if ($CURUSER["class"] >= UC_STAFF && $iphistory['ips'] > 0)
        $HTMLOUT .="<tr><td class='rowhead'>IP History</td><td align='left'>This user has earlier used <b><a href='{$TBDEV['baseurl']}/staffpanel.php?tool=iphistory&amp;action=iphistory&amp;id=" .$user['id'] ."'>{$iphistory['ips']} different IP addresses</a></b></td></tr>\n";
    ...
    $mc1->cache_value('port_data_'.$id, $port_data, $TBDEV['expires']['port_data']);
    }
    if($port_data > 0){
    $connect = $port_data[0];
    $port = $port_data[1];
    ...
    $invitedby[] = $invitee2;
    $mc1->cache_value('invited_by_'.$id, $invitedby, $TBDEV['expires']['invited_by']);
    }
    ...
    $mc1->cache_value('user_snatches_data_'.$id, $user_snatches_data, $TBDEV['expires']['user_snatches_data']);
    }
