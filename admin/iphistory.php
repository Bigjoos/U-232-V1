...
switch($do) {
	...
	$pager = pager($perpage,$count,'staffpanel.php?tool=iphistory&amp;action=iphistory&amp;id='.$uid.'&amp;'.(($order == 'access' ? 'order=access' : 'order=ip').'&amp;'));
	...
	<td class='colhead'><a class='colhead' href='{$TBDEV['baseurl']}/staffpanel.php?tool=iphistory&amp;action=iphistory&amp;id=".$uid."&amp;order=access'>Last access</a></td>\n
	<td class='colhead'><a class='colhead' href='{$TBDEV['baseurl']}/staffpanel.php?tool=iphistory&amp;action=iphistory&amp;id=".$uid."&amp;order=ip'>IP</a></td>\n
	...
	while ($a = mysql_fetch_assoc($q1))
	{
		...
		$ip = long2ip($a['ip']);
		if($a['log_count'] >= 1 )
			$HTMLOUT .= "<td><b><a href='{$TBDEV['baseurl']}/staffpanel.php?tool=ipsearch&amp;action=ipsearch&amp;ip=". $ip ."' title='ip used by other persons'>" . $ip ."</a></b></td>\n";
		elseif($a['ban_count'] > 0) 
			$HTMLOUT .= "<td><a href='{$TBDEV['baseurl']}/staffpanel.php?tool=testip&amp;action=testip&amp;ip=" . $ip . "' title='ip banned'><font color='#FF0000' ><b>" . $ip . "</b></font></a></td>\n";
		else
