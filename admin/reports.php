...
if ((isset($_GET["deal_with_report"])) || (isset($_POST["deal_with_report"]))) { 
   ...
   " . ($CURUSER["class"] == UC_SYSOP ? "<td class='colhead' align='center' valign='top'>Delete</td>" : "") . "</tr>";
    ...
    while ($arr_info = mysql_fetch_assoc($res_info)) {
        ...
        if ($arr_info["reporting_type"] != "") {
            switch ($arr_info["reporting_type"]) {
                ...
                case "Hit_And_Run":
                    ...
                    break;
                case "Post":
                    $res_who2 = sql_query("SELECT topic_name FROM topics WHERE id = " . $arr_info['2nd_value']);
                    $arr_who2 = mysql_fetch_assoc($res_who2);
                    $link_to_thing = "<b>post:</b> <a class='altlink' href='forums.php?action=view_topic&amp;topic_id=" . $arr_info['2nd_value'] . "&amp;page=last#" . $arr_info['reporting_what'] . "'><b>" . htmlspecialchars($arr_who2['subject']) . "</b></a>";
                    break;
            }
        }
        ...
        <td align='center' valign='middle' class='$class'>{$checkbox}</td>" . ($CURUSER["class"] == UC_SYSOP ? "<td align='center' valign='middle' class='$class'><a class='altlink' href='staffpanel.php?tool=reports&amp;action=reports&amp;id=" . $arr_info['id'] . "&amp;delete=1'><font color='red'>Delete</font></a></td>" : "") . "</tr>\n";
        // ===how was it delt with?
        if ($arr_info['how_delt_with'])
        $HTMLOUT .="<tr>
        <td colspan='" . ($CURUSER["class"] == UC_SYSOP ? "6" : "5") . "' class='$class' align='left'><b>Delt with by " . $arr_who['username'] . ":</b> " .get_date($arr_info['when_delt_with'], 'LONG',0,1) . "</td></tr>
