...
$days = 1; //number of days of inactivity
// end config
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($action == "deluser" && (!empty($_POST["userid"]))) {
        ...
        stderr($lang['inactive_success'], "{$lang['inactive_deleted']} <a href='".$TBDEV['baseurl']."/admin.php?action=inactive'>{$lang['inactive_back']}</a>");
    }
    if ($action == "disable" && (!empty($_POST["userid"]))) {
        sql_query("UPDATE users SET enabled='no' WHERE id IN (" . implode(", ", $_POST['userid']) . ") ");
        stderr($lang['inactive_success'], "{$lang['inactive_disabled']} <a href='".$TBDEV['baseurl']."/admin.php?action=inactive'>{$lang['inactive_back']}</a>");
    }
    ...
    if ($action == "mail" && (!empty($_POST["userid"]))) {
        ...
        while ($arr = mysql_fetch_array($res)) {
            ...
            {$lang['inactive_forgotten']} {$TBDEV['baseurl']}/resetpw.php\n
        ...
        if ($mail)
            stderr($lang['inactive_success'], "{$lang['inactive_msgsent']} <a href='".$TBDEV['baseurl']."/staffpanel.php?tool=inactive'>{$lang['inactive_back']}</a>");
