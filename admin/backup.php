...
$HTMLOUT.="<script type='text/javascript'>
 ...
 Click<a href='{$TBDEV['baseurl']}/staffpanel.php?tool=backup&amp;action=backup&amp;mode=backup'><b>&nbsp;Here&nbsp;</b></a>to backup now.<br />
 Click<a href='{$TBDEV['baseurl']}/staffpanel.php?tool=backup&amp;action=backup&amp;mode=check'><b>&nbsp;Here&nbsp;</b></a>to check config.</h1>";
 ...
 $res = sql_query('SELECT db.id, db.name, db.added, u.id AS uid, u.username '.
	...
	{
	$HTMLOUT.="<form method='post' action='staffpanel.php?tool=backup&amp;action=backup&amp;mode=delete'>
    ...
    <td class='colhead' style='white-space:nowrap;'>Added by</td>
		...
		{
		$HTMLOUT.="<tr>
			<td><a href='staffpanel.php?tool=backup&amp;action=backup&amp;mode=download&amp;id=".(int)$arr['id']."'>".htmlspecialchars($arr['name'])."</a></td>
     ...
     <input type='submit' value='Delete Selected' onclick=\"return confirm('Are you sure you want to delete the selected backups?');\" />
	   ...
	   $HTMLOUT.="<br />";
     stdmsg("Options", "<div align='center'><a href='staffpanel.php?tool=backup&amp;action=backup&amp;mode=backup'>Database Backup</a>&nbsp;&nbsp;-&nbsp;&nbsp;<a href='staffpanel.php?tool=backup&amp;action=backup&mode=check'>Settings Check</a></div>");
...
else if ($mode == "download")
{
	...
	header('Refresh: 0; url=staffpanel.php'.($autodl && !$autodel ? '' : '?tool=backup&amp;action=backup&mode=delete&id='.$id));
...
else if ($mode == 'delete')
{
	...
	header('Location:staffpanel.php?tool=backup&amp;action='.$location);
}
else if ($mode == "check")
{
	$HTMLOUT.= begin_main_frame();
