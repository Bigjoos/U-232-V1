...
class_check(UC_SYSOP);
	...
	if(isset($_GET['act'])){
		...
		if($ACT==1){//--EDIT
			...
			$HTML.="
			<form action='{$TBDEV['baseurl']}/staffpanel.php?tool=themes&amp;action=themes&amp;act=4' method='post'><input type='hidden' value='{$TEM['id']}' name='uri' /><table width='50%'>
		...
		if($ACT==2){//--DELETE
			...
			$ID = (int) $_GET['id'];
			if(!is_valid_id($ID))stderr("{$lang['themes_error']}", "{$lang['themes_inv_id']}");
			stderr("{$lang['themes_delete_q']}", "{$lang['themes_delete_sure_q']}<a href='{$TBDEV['baseurl']}/staffpanel.php?tool=themes&amp;action=themes&amp;act=5&amp;id=$ID&amp;sure=1'>
		...
		if($ACT==4){//--SAVE EDIT
			...
			header("Location: {$TBDEV['baseurl']}/staffpanel.php?tool=themes&action=themes&msg=1");
		}
      ...
      $ID = (int) $_GET['id'];
			...
			if($_POST['sure']!="1")header("Location: staffpanel.php?tool=themes");
			...
			header("Location: {$TBDEV['baseurl']}/staffpanel.php?tool=themes&action=themes&msg=2");
		}
		if($ACT==6){//--ADD NEW SAVE
			...
			"{$lang['themes_inv_file']}<a href='{$TBDEV['baseurl']}/staffpanel.php?tool=themes&amp;action=themes&amp;act=7&amp;id=$_POST[id]&amp;uri=$_POST[uri]&amp;name=$_POST[name]'>{$lang['themes_file_exists']}</a>/
			<a href='{$TBDEV['baseurl']}/staffpanel.php?tool=themes'>{$lang['themes_not_exists']}</a>");
			sql_query("INSERT INTO stylesheets(id, uri, name)VALUES(".sqlesc($_POST['id']).", ".sqlesc($_POST['uri']).", ".sqlesc($_POST['name']).")");
			header("Location: {$TBDEV['baseurl']}/staffpanel.php?tool=themes&action=themes&msg=3");
		}
	...
	if(!isset($_GET['act'])){
		$HTML.="<table width='80%'>
		<tr><td colspan='5'><a href='{$TBDEV['baseurl']}/staffpanel.php?tool=themes&amp;action=themes&amp;act=3'><span class='btn'>{$lang['themes_addnew']}</span></a></td></tr>
		<tr>
		...
		while($TE=mysql_fetch_array($TEMPLATES)){
			...
			<td align='left'><a href='{$TBDEV['baseurl']}/staffpanel.php?tool=themes&amp;action=themes&amp;act=1&amp;id=$TE[id]'>[{$lang['themes_edit']}]</a>
			<a href='{$TBDEV['baseurl']}/staffpanel.php?tool=themes&amp;action=themes&amp;act=2&amp;id=$TE[id]'>[{$lang['themes_delete']}]</a></td>
			</tr>
			";
		}
