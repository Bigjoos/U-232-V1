...
switch ($action2) 
  ...
  $check_if_there = sql_query('SELECT suspect FROM shit_list WHERE userid='.$CURUSER['id'].' AND suspect='.$shit_list_id);
  if (mysql_num_rows($check_if_there) == 1)
  stderr('Error', 'The member '.htmlspecialchars($arr_name['username']).' is already on your shit list!');
  ...
  $HTMLOUT .='<h1><img src="pic/smilies/shit.gif" alt="*" /> Add '.$arr_name['username'].' to your Shit List <img src="pic/smilies/shit.gif" alt="*" /></h1>
  ...
  if (!$sure)
    stderr('Delete '.htmlspecialchars($arr_name['username']).' from shit list',
         'Do you really want to delete <b>'.htmlspecialchars($arr_name['username']).'</b> from your shit list?  
   ...
   $mc1->delete_value('shit_list_'.$shit_list_id);
   $message = '<h1>Success! <b>'.htmlspecialchars($arr_name['username']).'</b> deleted from your shit list!</h1>';
...
$HTMLOUT .= $message.'
      ...
      <h1>Shit List for '.htmlspecialchars($CURUSER['username']).'</h1>
...
$i = 1;
   ...
   while ($shit_list = mysql_fetch_array($res))
   {
      ...
      {
      $shit .= ' <img src="pic/smilies/shit.gif" title="'.$shit_list['shittyness'].' out of 10 on the sittyness scale" alt="*" />';
      }
...
$HTMLOUT .= 
    (($i % 2 == 1) ? '<tr>' : '').'
      ...
      <a class="altlink" href="staffpanel.php?tool=shit_list&amp;action=shit_list&amp;action2=delete&amp;shit_list_id='.$shit_list['suspect_id'].'" title="remove this toad from your shit list">Remove</a>


      <a class="altlink" href="sendmessage.php?receiver='.$shit_list['suspect_id'].'" title="send a PM to this evil toad">Send PM</a></td>
      ...
      '.format_comment($shit_list['text']).'</td>'.
    (($i % 2 == 0) ? '</tr><tr><td class="colhead" align="center" colspan="4"></td></tr>' : '');
...
print stdhead('Shit list for '.htmlspecialchars($CURUSER['username'])) . $HTMLOUT . stdfoot();
?>