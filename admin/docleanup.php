...
while ($arr = mysql_fetch_assoc($res)) {
    switch ($arr['arg']) {
        ...
        case 'lastslowcleantime2': $arg = $TBDEV['autoslowclean_interval2'];
            break;
       case 'lastlottocleantime': $arg = $TBDEV['lotteryclean_interval'];
            break;
        case 'lastoptimizedbtime': $arg = $TBDEV['optimizedb_interval'];
            break;
        case 'lastbackuptime': $arg = $TBDEV['autobackup_interval'];
            break;
    }

    $HTMLOUT .="<tr><td>".$arr['arg']."</td>
    <td>".get_date($arr['value_u'], 'TINY',0,1)."</td>
    <td>" . calctime($arg) . "</td>
...
if (isset($_POST['docleanup'])) {
    ...
    docleanup();
    header('Refresh: 2; url='.$TBDEV['baseurl'].'/staffpanel.php?tool=docleanup');
