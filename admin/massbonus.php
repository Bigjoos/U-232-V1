		...
		stderr("Sorry","There are no users in the class(es) you selected");
			while($a = mysql_fetch_assoc($q1))
			{
				...
				if($sendpm)
				{
					...
					$body = sqlesc("Hey,\n we have decided to ". ($do == "remove_all" ?  "remove all seedbonus from your group class" : ($do == "add" ? "add $seedbonus Karma points".($seedbonus > 1 ? "s" : "")." to your group class" : "remove $seedbonus freeslot".($seedbonus > 1 ? "s" : "")."  from your group class")). " !\n ".$TBDEV['site_name'] ." staff");
