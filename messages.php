...
if (!$action)
{
    ...
    {
    $res = sql_query('SELECT name FROM pmboxes WHERE userid=' . sqlesc($CURUSER['id']) . ' AND boxnumber=' . sqlesc($mailbox) . ' LIMIT 1') or
    ...
    $pmcount = mysql_fetch_row(sql_query("SELECT COUNT(*) FROM messages WHERE receiver=".$CURUSER['id']." AND location >= '1' || sender=".$CURUSER['id']." AND saved = 'yes' ")) or sqlerr(__FILE__,__LINE__);
    ...
    <tr><td align='right'><a href='{$TBDEV['baseurl']}/messages.php?action=editmailboxes'>Add/Edit Mailboxes</a></td></tr>
    ...
    else
    {
      while ($row = mysql_fetch_assoc($res))
      {
        ...
        {
          $res2 = sql_query("SELECT username FROM users WHERE id=" . sqlesc($row['sender']));
        ...
        {
          $res2 = sql_query("select username FROM users WHERE id=" . sqlesc($row['receiver']));
          $username = mysql_fetch_array($res2);
          $username = "<a href='userdetails.php?id={$row['receiver']}'>{$username[0]}</a>";
 
        //Memcached - delete the keys
        //$mc1->delete_value('inbox_new_'.$row['receiver']);
        //$mc1->delete_value('inbox_new_sb_'.$row['receiver']);
 ...
 /*
    ...
    if ($action == "viewmessage")
    {
      ...
      //Memcached - delete the keys
        ...
        $mc1->delete_value('inbox_new_sb_'.$CURUSER['id']); // only need one cache
      }
    ...
    if ($action == "moveordel")
    {
      ...
      if (isset($_POST['move']))
      {
        ...
        else
        {
          ...
          @sql_query("UPDATE messages SET location=" . sqlesc($pm_box) . " WHERE id IN (" . implode(", ", array_map("sqlesc",$pm_messages)) . ') AND receiver=' .
      ...
      elseif ($_POST['delete'])
      {
        ...
        //$mc1->delete_value('inbox_new_sb_'.$row['receiver']); // only need one cache
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'GET')
    {
      ...
      else
      {
        ...
        @sql_query("INSERT INTO messages (poster, sender, receiver, added, subject, msg, location, saved) VALUES({$CURUSER["id"]}, {$CURUSER["id"]}, $to, " . time() . ", " . sqlesc($subject) . "," .
    ...
    if ($action == "editmailboxes2")
    {
      ...
      if ($action2 == "add")
      {
        ...
        {
          ++$box;
          @sql_query("INSERT INTO pmboxes (userid, name, boxnumber) VALUES (" . sqlesc($CURUSER['id']) . ", " . sqlesc($name1) . ", $box)") or sqlerr(__FILE__,__LINE__);
        }
        
        if (strlen($name2) > 0)
        {
          ++$box;
          @sql_query("INSERT INTO pmboxes (userid, name, boxnumber) VALUES (" . sqlesc($CURUSER['id']) . ", " . sqlesc($name2) . ", $box)") or sqlerr(__FILE__,__LINE__);
        }
        
        if (strlen($name3) > 0)
        {
          ++$box;
          @sql_query("INSERT INTO pmboxes (userid, name, boxnumber) VALUES (" . sqlesc($CURUSER['id']) . ", " . sqlesc($name3) . ", $box)") or sqlerr(__FILE__,__LINE__);
        }
...
function insertJumpTo($selected = 0)
    {
      ...
      while ($row = mysql_fetch_assoc($res))
      {
        ...
        {
          $htmlout .= "<option value='{$row['boxnumber']}' selected='selected'>".htmlspecialchars($row['name'])."</option>\n";
        }
        else
        {
          $htmlout .= "<option value='{$row['boxnumber']}'>".htmlspecialchars($row['name'])."</option>\n";
