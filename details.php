...
function ratingpic($num) {
    ...
    parked();
    $stdfoot = array(/** include js **/'js' => array('popup','jquery.thanks','java_klappe'));
    $HTMLOUT = '';
...
if (!$row || ($row["banned"] == "yes" && !$moderator))
   ...
   $HTMLOUT .= tr("{$lang['details_poster']}", "<a href=\"javascript: klappe_news('a2')\"><img border=\"0\" src=\"pic/plus.gif\" id=\"pica2\" alt=\"[Hide/Show]\" /></a><div id=\"ka2\" style=\"display: none;\"><img src='{$TBDEV['pic_base_url']}noposter.png' alt='Poster' title='Poster' /></div>", 1);
  ...
  //end auto imdd
    ...
    if ($CURUSER['class'] >= UC_MODERATOR) {
       if (!empty($row['checked_by'])) {
           ...
           <a href='{$TBDEV["baseurl"]}/details.php?id=".$row['id']."&amp;rechecked=1'><small><em><strong>[Re-Check this torrent]</strong></em></small></a> 
            <a href='{$TBDEV["baseurl"]}/details.php?id=".$row['id']."&amp;clearchecked=1'><small><em><strong>[Un-Check this torrent]</strong></em></small></a>  * STAFF Eyes Only *</td></tr>";
       }
       else {
       $HTMLOUT .="<tr><td class='rowhead'>Checked by</td><td align='left'><font color='#ff0000'><strong>NOT CHECKED!</strong></font> 
       <a href='{$TBDEV["baseurl"]}/details.php?id=".$row['id']."&amp;checked=1'>
   ...
   // end
	  ...
	  $HTMLOUT .= tr("Request reseed", $reseed,1);
		//==End
		$HTMLOUT .= "<tr><td class='rowhead'>Status update</td><td><input type='button' onclick='status_showbox(\"{$CURUSER['username']} is viewing details for torrent {$TBDEV['baseurl']}/details.php?id={$row['id']}\")' value='do it!'/></td></tr>";
		$HTMLOUT .= "</table>";
