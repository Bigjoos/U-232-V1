...
function torrenttable($res, $variant = "index") {
    global $TBDEV, $CURUSER, $lang, $free, $mc1;
    $htmlout = '';
    /** ALL FREE/DOUBLE **/
    foreach($free as $fl) {
    switch ($fl['modifier']) {
...
$all_free_tag = ($fl['modifier'] != 0 && ($fl['expires'] > TIME_NOW || $fl['expires'] == 1) ? ' <a class="info" href="#">
   ...
   $htmlout .= "<td class='colhead' align='center'><a href='{$TBDEV['baseurl']}/browse.php?{$oldlink}sort=9&amp;type={$link9}'>{$lang["torrenttable_uppedby"]}</a></td>\n";
    ...
    while ($row = mysql_fetch_assoc($res)) 
    {
        ...
        $sticky = ($row['sticky']=="yes" ? "<img src='{$TBDEV['pic_base_url']}sticky.gif' style='border:none' alt='Sticky' title='Sticky !' />" : "");
        ...
        $release_group = ($row['release_group']=="scene" ? "&nbsp;<img src='{$TBDEV['pic_base_url']}scene.gif' title='Scene' alt='Scene' style='border:none' />" : ($row['release_group']=="p2p" ? "&nbsp;<img src='{$TBDEV['pic_base_url']}p2p.gif' title='P2P' alt='P2P' />" : "")); 
       ...
       $htmlout .= "</tr>\n";
       $htmlout .="<tr id=\"kdescr" . $row["id"] . "\" style=\"display:none;\"><td width=\"100%\" colspan=\"12\">{$descr}</td></tr>\n";
       }
       $htmlout .= "</table>\n";
       return $htmlout;
       }  
