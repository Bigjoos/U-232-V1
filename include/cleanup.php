...
unset($updatetorrents);
  //=== Update karma seeding bonus... made nicer by devinkray :D
      ...
      if (mysql_num_rows($res) > 0) {
        while ($arr = mysql_fetch_assoc($res)) {
            if ($arr['tcount'] >= 1000)
                $arr['tcount'] = 5;
            $users_buffer[] = '(' . $arr['userid'] . ',0.250 * ' . $arr['tcount'] . ')';
        }
  ...
  //== Delete old backup's
  $days = 7;
  $backup_dir = 'include/backup'; //-- path to the backup folder
  ...
  function dolotterycleanup()
  {
   ...
   $q = mysql_query('SELECT t.user as uid, u.seedbonus, u.modcomment FROM tickets as t LEFT JOIN users as u ON u.id = t.user ORDER BY RAND() ') or sqlerr(__FILE__,__LINE__);
  ...
  if (preg_match('@^(CHECK|ANALYZE|REPAIR|OPTIMIZE)[[:space:]]TABLE[[:space:]]' . $tablename . '$@i', $sql))
   ...
   global $TBDEV, $queries;
    ...
    ignore_user_abort(1);
    /* Your db-globals */
    global $TBDEV, $queries, $CURUSER, $backupdir;
    ...
    */
    //== Windows mysqldump
    system(sprintf('c:\AppServ\MySQL\bin\mysqldump --opt -h %s -u %s -p%s %s > %s/%s/%s-%s-%s-%s.sql', $mysql_host, $mysql_user, $mysql_pass, $mysql_db, getenv('DOCUMENT_ROOT'), $backupdir, $mysql_db, $day, $month, $year));
    /*
    //== Liux mysqldump
    ...
    */
    $ext = $mysql_db.'-'.date('d').'-'.date('m').'-'.date('Y').".sql";
	  sql_query("INSERT INTO dbbackup (name, added, userid) VALUES (".sqlesc($ext).", ".sqlesc(time()).", ".$TBDEV['site']['owner'].")") or sqlerr(__FILE__, __LINE__);
