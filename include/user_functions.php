...
case ($ratio == 0): return;
break;
case ($ratio < 0.6): return ' <img src="'.$TBDEV['pic_base_url'].'smilies/shit.gif" alt=" Bad ratio :("  title=" Bad ratio :("/>';
break;
case ($ratio <= 0.7): return ' <img src="'.$TBDEV['pic_base_url'].'smilies/weep.gif" alt=" Could be better"  title=" Could be better" />';
break;
case ($ratio <= 0.8): return ' <img src="'.$TBDEV['pic_base_url'].'smilies/cry.gif" alt=" Getting there!" title=" Getting there!" />';
break;
case ($ratio <= 1.5): return ' <img src="'.$TBDEV['pic_base_url'].'smilies/smile1.gif" alt=" Good Ratio :)" title=" Good Ratio :)" />';
break;
case ($ratio <= 2.0): return ' <img src="'.$TBDEV['pic_base_url'].'smilies/grin.gif" alt=" Great Ratio :)" title=" Great Ratio :)" />';
break;
case ($ratio <= 3.0): return ' <img src="'.$TBDEV['pic_base_url'].'smilies/w00t.gif" alt=" Wow! :D" title=" Wow! :D" />';
break;
case ($ratio <= 4.0): return ' <img src="'.$TBDEV['pic_base_url'].'smilies/pimp.gif" alt=" Fa-boo Ratio!" title=" Fa-boo Ratio!" />';
break;
case ($ratio > 4.0): return ' <img src="'.$TBDEV['pic_base_url'].'smilies/yahoo.gif" alt=" Great ratio :-D" title=" Great ratio :-D" />';
break;
  }
  return '';
}
 ...
 function avatar_stuff($avatar, $width = 80) {
  global $CURUSER;
  $avatar_show = ($CURUSER['avatars'] == 'no' ? '' : (!$avatar['avatar'] ? '<img style="max-width:'.$width.'px;" src="pic/default_avatar.gif" alt="avatar" />' : 
  (($avatar['offensive_avatar'] === 'yes' && $CURUSER['view_offensive_avatar'] === 'no') ? 
  '<img style="max-width:'.$width.'px;" src="pic/fuzzybunny.gif" alt="avatar" />' : '<img style="max-width:'.$width.'px;" src="'.htmlspecialchars($avatar['avatar']).'" alt="avatar" />')));   
  return $avatar_show;
}
  ...
  return '<span style="white-space:nowrap;"><a href="member_details.php?id='.$arr['id'].'" title="'. get_user_class_name($arr['class']).'">
