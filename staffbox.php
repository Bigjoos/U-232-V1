 ...
 **/
    ...
    $stdfoot = array(/** include js **/'js' => array('staffcontact'));
	...
	case 'setanswered' : 
		if($id > 0) {
			...
			$q1 = mysql_query('SELECT s.msg,s.sender,s.subject,u.username FROM staffmessages as s LEFT JOIN users as u ON s.sender=u.id WHERE s.id IN ('.join(',',$id).')') or sqlerr(__FILE__,__LINE__);
			$a = mysql_fetch_assoc($q1);
			$response = htmlspecialchars($message)."\n---". $a['username']." wrote ---\n".$a['msg'];
			sql_query('INSERT INTO messages(sender,receiver,added,subject,msg) VALUES('.$CURUSER['id'].','.$a['sender'].','.time().','.sqlesc('RE: '.$a['subject']).','.sqlesc($response).')') or sqlerr(__FILE__,__LINE__);
			$mc1->delete_value('inbox_new_'.$a['sender']);
      $mc1->delete_value('inbox_new_sb_'.$a['sender']); // only need one cache	
