...
loggedinorreturn();
   ...
   $descr = unesc($_POST["descr"]);
    ...
    $dict['value']['comment'] = bdec(benc_str("In using this torrent you are bound by the {$TBDEV['site_name']} Confidentiality Agreement By Law")); // change torrent comment
    ...
    $url = unesc($_POST['url']);
    $poster = unesc($_POST['poster']);
    $ret = sql_query("INSERT INTO torrents (search_text, filename, owner, visible, release_group, poster, anonymous, allow_comments, info_hash, name, size, numfiles, type, url, descr, ori_descr, category, free, save_as, added, last_action, nfo, client_created_by) VALUES (" .
        implode(",", array_map("sqlesc", array(searchfield("$shortfname $dname $torrent"), $fname, $CURUSER["id"], "no", $release_group, $poster, $anonymous, $allow_comments, $infohash, $torrent, $totallen, count($filelist), $type, $url, $descr, $descr, 0 + $_POST["type"], $free, $dname))) .
