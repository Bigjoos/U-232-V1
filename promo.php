...
if ($_SERVER["REQUEST_METHOD"] == "POST" && $do == "addpromo")	
	{
		...
		$q = mysql_query("INSERT INTO promo (name,added,days_valid,max_users,link,creator,bonus_upload,bonus_invites,bonus_karma) VALUES (".implode(",", array_map("sqlesc",array($promoname,time(),$days_valid,$max_users,$link,$CURUSER["id"],$bonus_upload,$bonus_invites,$bonus_karma))).") ") or sqlerr(__FILE__, __LINE__);
			...
			stderr("Success","The promo link <b>".$promoname."</b> was added! here is the link <br /><input type=\"text\" name=\"promo-link\" value=\"".$TBDEV['baseurl'].$_SERVER["PHP_SELF"]."?do=signup&amp;link=".$link."\" size=\"80\" onclick=\"select();\"  /><br/><a href=\"".$_SERVER["PHP_SELF"]."\"><input type=\"button\" value=\"Back to Promos\" /></a>");
	}
	elseif($_SERVER["REQUEST_METHOD"] == "POST" && $do == "signup")
	{
			...
			else
			{
				...
				stderr("Error", "That dosen't look like an email adress");
					...
					$var_check = mysql_query("SELECT id, editsecret FROM users where username=".sqlesc($username)." OR email=".sqlesc($email))or sqlerr(__FILE__, __LINE__);
          ...
          $passhash = make_passhash( $secret, md5($password) );
					...
					$res = mysql_query("INSERT INTO users(username, passhash, secret, editsecret, email, added, uploaded, invites, seedbonus) VALUES (".implode(",",array_map("sqlesc",array($username,$passhash,$secret,$editsecret,$email,time(),($ar_check["bonus_upload"]*1073741824),$ar_check["bonus_invites"],$ar_check["bonus_karma"]))).") ") or sqlerr(__FILE__, __LINE__);
					if($res)
					{
						...
						//==Email part :)
						$md5 = md5($editsecret);
						$subject = $TBDEV['site_name']." user registration confirmation";
						
						$message = "Hi!
						You used the link from promo ".$ar_check["name"]." and registred a new account at {$TBDEV['site_name']}
							
						To confirm your account click the link below
						{$TBDEV['baseurl']}/confirm.php?id=$userid&secret=$md5

						Welcome and enjoy your stay 
						Staff at {$TBDEV['site_name']}";
						
						
						$headers = 'From: ' . $TBDEV['site_email'] . "\r\n" .'Reply-To:' . $TBDEV['site_email'] . "\r\n" .'X-Mailer: PHP/' . phpversion();
	...
	{
		$r = mysql_query("SELECT name FROM promo WHERE id=".$id) or sqlerr(__FILE__, __LINE__);
		if($sure == "no" ){
		...
		elseif( $sure == "yes")
		{
			if(mysql_query("DELETE FROM promo where id=".$id)or sqlerr(__FILE__, __LINE__))
			{
	...
	elseif ($do == "addpromo")
	{
			...
			$HTMLOUT .= end_frame();
	    print stdhead('Add Promo Link') . $HTMLOUT . stdfoot();
	}
	elseif ($do =="signup")
	{
			if(empty($link))
			...
			else
			{
				$r_promo =  mysql_query("SELECT * from promo where link=".sqlesc($link)) or sqlerr(__FILE__, __LINE__);
					...
					else 
					{
						...
						$HTMLOUT .= end_frame();
					print stdhead("Signup for promo :".$ar["name"]) . $HTMLOUT . stdfoot();
					}
			}
	}
	elseif ($do == "accounts")
	{
		...
		$HTMLOUT .="<table align='center' width='100%' cellpadding='5' cellspacing='0' border='1' style='border-collapse:collapse'>
		  ...
		  $active = (($ar["max_users"] == $ar["accounts_made"]) || (($ar["added"] + (86400*$ar["days_valid"])) < time())) ? false : true;
			...
			$HTMLOUT .="<tr ".(!$active ? "title=\"This promo has ended\"" : "" ).">
				<td nowrap='nowrap' align='center'>".(htmlspecialchars($ar["name"]))."<br /><input type='text' ".(!$active ? "disabled=\"disabled\"" : "")." value='".($TBDEV['baseurl'].$_SERVER["PHP_SELF"]."?do=signup&amp;link=".$ar["link"])."' size='60' name='".(htmlspecialchars($ar["name"]))."' onclick='select();' /></td>
		...
		$HTMLOUT .="</table>";
		$HTMLOUT .= end_frame();
		print stdhead("Current Promos") . $HTMLOUT . stdfoot();
		}
	}
?>