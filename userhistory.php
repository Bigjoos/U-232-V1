...
loggedinorreturn();
    ...
    if ($action == "viewposts")
    {
      ...
      $where_is = "p.user_id = $userid AND f.min_class_read <= " . $CURUSER['class'];
      ...
      $res = sql_query("SELECT id, username, class, donor, warned, leechwarn, pirate, king, chatpost, enabled FROM users WHERE id=$userid") or sqlerr(__FILE__, __LINE__);
      ...
      while ($arr = mysql_fetch_assoc($res))
      {
          ...
          $posterid = $arr["user_id"];

          $topicid = $arr["t_id"];

          $topicname = $arr["topic_name"];
        ...
        $added = get_date( $arr['added'],'');
          ...
          &nbsp;--&nbsp;<b>{$lang['posts_post']}:&nbsp;</b>
          #<a href='forums.php?action=view_topic&amp;topic_id=$topicid&amp;page=p$postid#$postid'>$postid</a>" .
          ...
          {
              $subres = sql_query("SELECT username FROM users WHERE id=$arr[edited_by]");
              if (mysql_num_rows($subres) == 1)
              {
                  $subrow = mysql_fetch_assoc($subres);
                  $body .= "<p><font size='1' class='small'>{$lang['posts_lasteditedby']} <a href='userdetails.php?id=$arr[edited_by]'><b>$subrow[username]</b></a> {$lang['posts_at']} ".get_date($arr['edit_date'], 'LONG',0,1)."</font></p>\n";
              }
          }
    ...
    if ($action == "viewcomments")
    {
      ...
      $res = sql_query("SELECT id, class, username, donor, warned, leechwarn, chatpost, pirate, king, enabled FROM users WHERE id=$userid") or sqlerr(__FILE__, __LINE__);
